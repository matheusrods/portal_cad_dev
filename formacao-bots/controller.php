...
Class funcoes {
    ...
    public function gravarConversa($idConversa, $idUsuario, $inputUsuario, $respostaBot, $contextoConversa){
        ...
        $query = "
            INSERT INTO `formacaoBots`.`logConversas` (`idConversa`, `usuario`, `input`, `resposta_bot`, `contexto`) 
        ...
        try{
            ...
            $arquivoLog = $this->geraLogExcecao("Log Conversa Bot Formação", "gravaConversa", $informacoesErro, $mat);
    ...
    public function consultarContexto(){
        ...
        $db = New Database('formacaoBots');
        $query = "
            SELECT contexto FROM formacaoBots.logConversas WHERE usuario = '".$mat."' AND CURDATE() = date(timestamp) ORDER BY timestamp DESC LIMIT 1;
        ";
        try{
            ...
            $informacoesErro = "Erro: " . $e . "\n\n\$query: " . $query;
            $arquivoLog = $this->geraLogExcecao("Consulta Contexto Bot Formação", "consultarContexto", $informacoesErro, $mat);
            $retorno = "Erro ao gravar log. Arquivo: " . $arquivoLog." - Matricula: ".$mat;
        } finally {
            return ($retorno);
        }
