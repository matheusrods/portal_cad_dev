$(document).ready(function() {
  ...
  //Ao clicar na imagem do organograma do CAD a página interna "Quem Somos é aberta"
    ...
    function abrirPaginaInterna(linkInterno){
        var caminhoPages = 'https://'+host+'/lib/apps/'+linkInterno+'/app/'+linkInterno+'.php';
        var decoracaoNatal = '';

        if(queDiaEhHoje() <= "2024-12-31") {
            decoracaoNatal = '<div class="natal" style="background-image: url(https://'+host+'/lib/img/cabecalho/natal2.gif); background-repeat: repeat-x; width: 140%; height: 10vh; background-size: 500px; position: absolute; margin: -0.5rem -5rem; z-index: 3;"></div>';
        }

        $.ajax({
            url: 'https://'+host+'/lib/class/validaSessao.php?request=validaSessao',
            method: 'GET',
            dataType: 'json',
