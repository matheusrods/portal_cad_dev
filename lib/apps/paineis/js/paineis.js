$(document).ready(function() {
    ...
    $(".botaoVerMaisPaineis").on('click', function (event) {
        ...
        var $linha = $(".abreDivNestPainel[attr-sequencia="+novoSequencial+"]");

        if ($linha.length > 0) {
            var offset = $linha.offset();
            $('html,body').animate({
    ...
    // Interação para incluir/excluir paineis como favoritos
    $('.favoritarPainel').on('click', function(){
        alert('Fazer inclusão/exclusão do favorito');
    ...
    function consultaPaineis(idTag){
        var caminhoController = 'https://cad.bb.com.br/lib/apps/paineis/controller/controller_paineis.php';
        $.ajax({
            aSync: true,
            ...
            error: function(erro) {
                alert("Não foi possível efetuar a operação, por favor tente novamente. L189 - paineis.js");
            }
        });
    }
    ...
    function pesquisaPaineis(textoDigitado){
        ...
        $.ajax({
            ...
            error: function(erro) {
                alert("Não foi possível efetuar a operação, por favor tente novamente. L238 - paineis.js");
            }
        });
    }
});