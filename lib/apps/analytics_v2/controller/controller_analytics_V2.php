...
switch($request){
    ...
    case "consultaGrandesNumerosPf":
        $retorno = $class->consultaGrandesNumerosPf($dataInicio, $dataFim);
        echo json_encode($retorno);
    break;
    
    case "consultaGrandesNumerosPj":
        $retorno = $class->consultaGrandesNumerosPj($dataInicio, $dataFim);
        echo json_encode($retorno);
    break;

    case "consultaNumerosAcumulados":
        $retorno = $class->consultaNumerosAcumulados($dataInicio, $dataFim);
        echo ($retorno);
    break;

    case "atualizaGrandesNumerosPf":
        $retorno = $class->atualizaGrandesNumerosPf($dataInicio, $dataFim);
        echo json_encode($retorno);
    break;

    case "atualizaGrandesNumerosPj":
        $retorno = $class->atualizaGrandesNumerosPj($dataInicio, $dataFim);
        echo json_encode($retorno);
    break;
}
?>