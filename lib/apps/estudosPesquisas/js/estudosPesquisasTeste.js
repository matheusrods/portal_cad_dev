$(document).ready(function() {
    ...
    $('.botaoAdicionaEstudo').click(function(){
        var caminhoController = 'https://cad.desenv.bb.com.br/lib/apps/estudosPesquisas/controller/controller_estudosPesquisas.php';
        $.ajax({
            aSync: true,
        ...
        $.ajax({
            ...
            success: function(retorno) {
                if (retorno.status == 1) {
                    ...
                    $(".abreDivNest"+qualOpcaoMaiuscula).css({"display": "inline-flex", "justify-content": "center", "gap": "20px"});
    ...
    $('.inputCampoPesquisa').on('input', function() {
        ...
        if ($('#abaEstudos').is(':visible')) {
            if (termo === '') {
                $('.quadroConteudoEstudos .divEstudo').hide().slice(0, 4).show();
        ...
        if ($('#abaPesquisas').is(':visible')) {
            if (termo === '') {
                $('.quadroConteudoPesquisas .divPesquisa').hide().slice(0, 4).show();
