...
class cabecalho{
    ...
    public function montaCabecalho(){
        ...
        for($i=0; $i < sizeof($execQuery); $i++){
            ...
            $cabecalho = $cabecalho.'<div id="'.$execQuery[$i]['id'].'" class="divCabecalho '.$classLinkInterno.' Clicar" '.$nomePaginaInterna.'><span class="textoMenuCabecalho">'.$execQuery[$i]['item'].'</span></div>';
        }
        ...
        for($i=0; $i<sizeof($totalItens); $i++){
            ...
            for($j=0; $j<sizeof($execQuery); $j++){
                ...
                $divItem = $divItem.$linhaVertical.'
                    <div class="subItem Clicar" attr-linkInterno="'.$execQuery[$j]['url'].'" style="width: '.$larguraDiv.'%; height: 3rem; justify-content: center; align-items: center; gap: 8px; display: flex">
            ...
            $divItemCompleta = $divItemCompleta.
                '<div id="item'.$totalItens[$i]['vinculoItem'].'"style="display: none; margin-top: 1rem; background-color: #FEFEFE;">
                    <div style="width: 100%; height: 100%; padding: 16px 32px 16px 32px; opacity: 0.90; background: #FEFEFE; border-top-left-radius: 8px; border-top-right-radius: 8px; justify-content: flex-start; align-items: flex-start; display: inline-flex">'
                        .$divItem.
                    '</div>
                </div>';
        }
