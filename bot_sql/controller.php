...
Class funcoes {
    ...
    public function gravarConversa($idConversa, $idUsuario, $inputUsuario, $respostaBot, $contextoConversa){
        ...
        $query = "
            INSERT INTO `logsBotsCad`.`logBotSql` (`idConversa`, `usuario`, `input`, `resposta_bot`, `contexto`) 
        ...
        try{
            ...
            $arquivoLog = $this->geraLogExcecao("Log Bot sql", "gravaConversa", $informacoesErro, $mat);
    ...
    public function consultarContexto(){
        ...
        $db = New Database('logsBotsCad');
        $query = "
            SELECT contexto FROM logsBotsCad.logBotSql WHERE usuario = '".$mat."' AND CURDATE() = date(timestamp) and ativo = 1 ORDER BY timestamp DESC LIMIT 1;
        ";
        try{
            ...
            $informacoesErro = "Erro: " . $e . "\n\n\$query: " . $query;
            $arquivoLog = $this->geraLogExcecao("Consulta Contexto Bot sql", "consultarContexto", $informacoesErro, $mat);
            $retorno = "Erro ao gravar log. Arquivo: " . $arquivoLog." - Matricula: ".$mat;
        } finally {
            return ($retorno);
        }
    ...
    public function zerarContexto($idConversa){
        ...
        $db = New Database('logsBotsCad');
        $query = "
            UPDATE `logsBotsCad`.`logBotSql` SET `ativo` = '0' WHERE (`idConversa` = '".$idConversa."');
        ";
        try{
            $execQuery = $db->DbGetAll($query);
            if($execQuery){
